...
class SkillsLoader:
            ...
            for file in files:
                if file == "SKILL.md":
                    ...
                    self.skills.append({
                        ...
                        "always_load": meta.get("always_load", False), # 是否强制全局加载
    ...
    def get_always_skills_prompt(self) -> str:
        ...
        """
        always_skills = [s for s in self.skills if s.get("active", True) and s.get("always_load", False)]
    ...
    def build_skills_summary_prompt(self) -> str:
        ...
        """
        available_skills = [s for s in self.skills if s.get("active", True) and not s.get("always_load", False)]
        if not available_skills:
            return ""

        prompt_parts = ["# 可选扩展技能 (Available Skills)"]
        prompt_parts.append("以下技能扩展了你的能力。想使用某项技能前，请务必使用 `read_file` 工具读取相应路径下的 SKILL.md 文件学习具体用法。\n")
    ...
    def get_skills_summary(self) -> list:
        """为前端状态展示提取所有的技能摘要列表"""
        return [{"name": s["name"], "description": s["description"], "active": s["active"]} for s in self.skills]
